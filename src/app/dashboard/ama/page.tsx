...
//   const loadINFTs = async () => {
//     setLoading(true);
//     try {
//       const response = await fetch(`${API_BASE}/list_infts`);
//       const data = await response.json();
//       setINFTs(data);
//     } catch (error) {
//       alert(`Error loading iNFTs: ${error.message}`);
//     }
//     setLoading(false);
//   };
...
export default function AMAChat() {
  ...
  const handleSend = async () => {
    ...
    try {
      const payload: ChatRequest = { user_id: String(myINFT.id), message: input };
  ...
  if (!myINFT || !otherINFT) {
    return (
      <div className="flex items-center justify-center h-screen text-white text-xl">
        {loading ? "Loading iNFT chat..." : "No iNFTs found."}
      </div>
    );
  }
